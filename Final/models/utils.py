...
def build_DiD_graph(treatment_df, control_df, policy_date='2018-01-01', rolling_window=30):
    treatment_df['datetime'] = pd.to_datetime(treatment_df['Date (LT)'])
    control_df['datetime'] = pd.to_datetime(control_df['Date (LT)'])
    ...
    combined = pd.DataFrame({
        'Delhi': treatment_df['AQI_smooth'],
        'Chennai': control_df['AQI_smooth']
    }).dropna()
